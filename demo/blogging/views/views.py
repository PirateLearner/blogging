...
def teaser(request,slug=None):
	...
	try:
		section = Section.objects.get(slug = str(current_section))
	...
	if request.GET.get('strict', None) is None:
		...
		nodes = Content.published.filter(section=section)
...
def edit_post(request,post_id):
	...
	try:
		blog = Content.objects.get(pk=post_id)
		...
		print "LOGS: Layout ", blog.content_type.__str__().lower()
		if request.method == "POST":
...
def edit_section(request,section_id):
	...
	try:
		...
		print "LOGS: Layout ", section.content_type.__str__().lower()
		if request.method == "POST":
...
def section_index(request, slug=None):
	...
	if current_section is not None:
		...
		nodes = Section.objects.filter(level=0)
...
def detail(request, slug, post_id):
	...
	try:
		...
		try:
			blogs = Content.objects.get(pk=post_id)
...
def get_posts_for_section(section=None):
	...
	if section is not None:
		...
		for child in children:
			qs |= Q(section=child)
		posts = Content.published.filter(qs)
	else:
...
def tagged_post(request,tag):
	try:
		posts = Content.objects.filter(tags__slug = tag)
...
# 		course = Section.objects.get(title='Course')
		section = None
		try:
			parent  = request.GET.get('section',None)
			print "Parent id from request ", parent
			if parent:
				section = Section.objects.get(pk=parent)
		except:
			section = None 
		
		context = RequestContext(request, {
# 	                                       'nodes': course.get_descendants(include_self=True),
											'nodes': Section.objects.filter(~Q(title="Orphan")),
	                                       'active': section 
	                                      })
...
def testCase(request):
	...
	course = Section.objects.get(title='Course')
	Django = Section.objects.get(title='Computer Science') 
...
def manage(request):
	...
	try:
		if request.method == "POST":
			...
			if len(pks):
#				 print "LOGS: " + pks
				objs = Content.objects.filter(pk__in=pks)
